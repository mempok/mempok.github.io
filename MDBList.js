...
(function () {
    ...
    if (window.Lampa && Lampa.SettingsApi) {
        ...
        // 3.1 Add Kinopoisk API Key (optional)
        Lampa.SettingsApi.addParam({
            component: 'additional_ratings',
            param: {
                ...
                'default': '',
                values: {},
                placeholder: 'Enter your Kinopoisk API Key'
            },
            field: {
                name: 'KinoPoisk API Key',
                description: 'Ключ для https://kinopoiskapiunofficial.tech/'
            },
            onChange: function() {
                Lampa.Settings.update();
            }
    ...
    function fetchKinopoiskRating(movieData, callback) {
        try {
            ...
            if (imdbId) {
                ...
                xhr.setRequestHeader('X-API-KEY', kpApiKey);
    ...
    function showRatingProviderSelection() {
        ...
        const providers = [
            ...
            { title: 'KinoPoisk', id: 'show_rating_kp', default: false },
    ...
    function create() { 
        ...
        this.update = function(data) { // Called on focus change with new movie data
            ...
            if (data.id && data.method) {
                ...
                // --- KinoPoisk Rating Fetch (optional, via kinopoiskapiunofficial.tech) ---
                var showKpToggle = Lampa.Storage.get('show_rating_kp', false);
        ...
        this.draw = function (data) {
            ...
            // KP toggle
            let kpStored = Lampa.Storage.get('show_rating_kp', false);
            ...
            // --- Get KP rating (from external API cache) ---
            var kpCached = kpRatingsCache[data.id];
            ...
            if (showKp) {
                // Prefer external cache (more надёжно), fallback к mdblistResult (если вдруг появится)
                let raw = kpCached && typeof kpCached.kp === 'number' ? kpCached.kp : null;
                if (raw == null && mdblistResult) raw = mdblistResult.kp ?? mdblistResult.kinopoisk ?? mdblistResult.kp_rating;
                let parsed = parseFloat(raw);
                if (!isNaN(parsed)) {
                    let score = parsed.toFixed(1);
                    if (parsed >= 0) {
                        lineOneDetails.push('<div class="full-start__rate kp-rating-item">' + '<div class="kp-score">' + score + '</div>' + '<img src="' + kpLogoUrl + '" class="rating-logo kp-logo" alt="KinoPoisk" draggable="false">' + '</div>');
                    }
                }
            }
            if (showAudience) {
